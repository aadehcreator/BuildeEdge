...
import { Plus, Loader2, ToggleLeft, ToggleRight, Trash2, PackagePlus, Sparkles, Store, ShieldAlert, ExternalLink, CheckCircle2, Upload } from 'lucide-react';
...
export default function VendorProductsPage() {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      fd.append('file', file);
      fd.append('folder', 'buildedge/products');
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
            ...
            <div className="sm:col-span-2">
              <div className="flex items-center justify-between mb-1">
                <label className={lbl}>Product Image (फोटो)</label>
                <button
                  type="button"
                  onClick={() => fileInputRef.current?.click()}
                  disabled={uploadingImage}
                  className="flex items-center gap-1.5 px-3 py-1 bg-primary/20 hover:bg-primary/30 text-primary rounded-lg text-xs font-semibold transition-colors disabled:opacity-50"
                >
              ...
              <input value={form.images[0]} onChange={(e) => setForm((p) => ({ ...p, images: [e.target.value] }))}
                placeholder="https://images.unsplash.com/photo-... or upload above" className={inp} />
              {form.images[0] && (
                <div className="mt-2 relative w-20 h-20 rounded-xl overflow-hidden border border-gray-700 bg-gray-800">
